...
def dnn_model(features, mode, params):
    X = features[TIMESERIES_COL]
    # DNN model
    layer_1 = tf.layers.dense(X, 10, activation=tf.nn.relu)
    layer_2 = tf.layers.dense(layer_1, 5, activation=tf.nn.relu)
    predictions = tf.layers.dense(layer_2, 1, activation=tf.nn.sigmoid)
    return predictions
...
def cnn_model(features, mode, params):
    ...
    # CNN model
    c1 = tf.layers.conv1d(X, filters=100, kernel_size=10, padding='same', activation=tf.nn.relu)
    p1 = tf.layers.max_pooling1d(c1, pool_size=2, strides=1)
    dr = tf.layers.dropout(p1, rate=0.2)
    c2 = tf.layers.conv1d(dr, filters=5, kernel_size=5, padding='same', activation=tf.nn.relu)
    p2 = tf.layers.max_pooling1d(c2, pool_size=2, strides=1)
    f = tf.layers.flatten(p2)
    d = tf.layers.dense(f, 5, activation=tf.nn.relu)
    predictions = tf.layers.dense(d, 1, activation=tf.nn.sigmoid)
...
def rnn_model(features, mode, params):
    ...
    # define RNN cell_type
    CELL_SIZE = N_INPUTS // 3
    cell = tf.nn.rnn_cell.GRUCell(CELL_SIZE)

    # instantiate dynamic_rnn
    outputs, state = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32)

    # extract final state and fit to dnn
    dense_1 = tf.layers.dense(state, 10, activation=tf.nn.relu)
    dense_2 = tf.layers.dense(dense_1, 3, activation=tf.nn.relu)
    predictions = tf.layers.dense(dense_2, 1, activation=tf.nn.sigmoid)
...
def rnn2_model(features, mode, params):
    ...
    print('rnn_model: state: {}'.format(state))  # info for debugging

    # extract final state and fit to dnn
    dense_1 = tf.layers.dense(state[1], 10, activation=tf.nn.relu)
    dense_2 = tf.layers.dense(dense_1, 3, activation=tf.nn.relu)
    predictions = tf.layers.dense(dense_2, 1, activation=tf.nn.sigmoid)
...
def read_dataset(filename, mode, batch_size=512):
    def _input_fn():
        def decode_csv(row):
            ...
            features = tf.decode_csv(row,
                                     record_defaults=DEFAULTS)  # string tensor -> list of 50 rank 0 float tensors
